...
def duration_masslost(agbs, justprint=False, norm=False):
    ...
    if justprint:
        ...
        for a in aa:
            for agb in agbs:
                ...
                if None not in [ind1, ind2] or ind1 != ind2:
                    if agb.mass != a:
                        continue
                    age = agb.data[age] / 1e5
                    mass = agb.data[mass]
                    print(ind1, ind2)
                    print('{:g} {:g} {:.2f} {:.2f} {:.2f} {:.2f}'
                          .format(agb.Z, agb.mass, age[ind1],
                                  age[ind2]-age[ind1],
                                  age[-1]-age[ind2], age[-1]))
        ...
        if agb.mass >= 3.2 and agb.mass <= 5.:
            for i, col in enumerate([col1, col2]):
                ...
                if agb.Z == 0.001 and agb.mass == 5. and i == 0:
                    ax.barh(agb.mass, tpd, 0.2, color=colors[0],
                            label=r'$\dot{M}_{pd}$', **kw)
                    ax.barh(agb.mass, tdd, 0.2, color=colors[1],
                            label=r'$\dot{M}_{dd}$', left=tpd, **kw)
                    ax.barh(agb.mass, tsw, 0.2, color=colors[2],
                            label=r'$\dot{M}_{sw}$', left=tdd+tpd, **kw)
                    if norm:
                        loc = 'upper left'
                        frameon = True
                    else:
                        loc = 'upper right'
                        frameon = False
                    ax.legend(labelspacing=0.02, loc=loc, frameon=frameon,
                    ...
                    if i > 0:
                        ...
                        if norm:
                            ttp = agb.mass
                        tpd = np.sum(agb.data['dt'][ipd] *
                                     agb.data['dMlost'][ipd]) / ttp
                        tdd = np.sum(agb.data['dt'][idd] *
                                     agb.data['dMlost'][idd]) / ttp
                        tsw = np.sum(agb.data['dt'][isw] *
                                     agb.data['dMlost'][isw]) / ttp
                        if norm:
                            ax.set_xlim(0, 1)
                        if agb.mass == 5.:
                            ax.text(0.98, 0.02,
                                    r'$\rm{{Z}}={0:g}$'.format(agb.Z),
                                    fontsize=16,
    ...
    fig.text(0.03, 0.58, r'$\rm{TP-AGB\ Initial\ Mass (M_\odot)}$',
             rotation='vertical', ha='center', va='center', fontsize=20)
    fig.subplots_adjust(left=0.1, hspace=0.05, wspace=0.05, right=0.92,
                        bottom=0.2, top=0.98)
    if norm:
...
def compare_vw93(agbs, outfile=None, xlim=None, ylims=None):
    """plot similart to VW1993 multipanel plot"""
    ...
    for i, _ in enumerate(col1):
        ...
        ax2 = col2[i]
        for ax in [ax1, ax2]:
            ax.set_ylim(minmax(ax1.get_ylim(), ax2.get_ylim()))
            ax.set_xlim(minmax(ax1.get_xlim(), ax2.get_xlim()))
...
def main(argv):
    desc = "Make a plot like Vassiliadis and Wood 1993"
